...
from taddyn.utils.extraviews     import plot_2d_optimization_result
# from taddyn.modelling.structuralmodels import StructuralModels
from taddyn.utils.modelAnalysis     import get_contact_matrix
from taddyn.utils.modelAnalysis     import correlate_with_real_data
...
class IMPoptimizer(object):
    ...
    :param experiment: an instance of the class taddyn.experiment.Experiment
                ...
                for i in xrange(len(self.zscores)):
                    if self.tool=='lammps':
                        tdm = generate_lammps_models(self.zscores, self.resolution, self.nloci,
                                          ...
                                          cleanup=cleanup, initial_conformation='random' if not initial_conformation \
                        ...
                        raise Exception(('ERROR: Tool must be lammps'))

                    result = 0
                    matrices = get_contact_matrix(tdm,
                        #cutoff=[int(dc * self.resolution * float(scale)) for dc in dcutoff_arange])
                        cutoff=[int(dc) for dc in dcutoff_arange])
                    for m in matrices:
                        cut = int(m**0.5)
                        sub_result = correlate_with_real_data(tdm, cutoff=cut, corr=corr,
                                                                  off_diag=off_diag,
    ...
    def get_best_parameters_dict(self, reference=None, with_corr=False):
        ...
        :returns: a dict that can be used for modelling, see config parameter in
           :func:`taddyn.experiment.Experiment.model_region`
    ...
    def write_result(self, f_name):
        ...
        the function taddyn.modelling.impoptimizer.IMPoptimizer.load_from_file
    ...
    def load_from_file_OLD(self, f_name):
        ...
        taddyn.modelling.impoptimizer.IMPoptimizer.write_result.
...
#         zscores=svd['zscore'])
#     try:
#         result = correlate_with_real_data(tdm,
