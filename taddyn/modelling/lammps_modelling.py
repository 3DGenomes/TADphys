...
from taddyn.modelling import LAMMPS_CONFIG as CONFIG
...
def generate_lammps_models(zscores, resolution, nloci, start=1, n_models=5000,
    ...
    :param None config: a dictionary containing the standard
       ...
       ::
         ...
         from taddyn.modelling.HIC_CONFIG import HIC_CONFIG
    ...
    models, ini_model = lammps_simulate(lammps_folder=tmp_folder, run_time=run_time,
...
def run_lammps(kseed, lammps_folder, run_time,
    ...
    if time_dependent_steering_pairs:
        ...
        # Performing the adaptation step from initial conformation to TADdyn excluded volume
        if time_dependent_steering_pairs['adaptation_step']:
            ...
            for time_point in time_points[0:1]:
                ...
                if to_dump:
                    lmp.command("undump 1")
                    lmp.command("dump    1       all    custom    %i  %sadapting_MD_from_initial_conformation_to_TADdyn_at_time_point_%s.XYZ  id  xu yu zu" % (to_dump, lammps_folder, time_point))
                    lmp.command("dump_modify     1 format line \"%d %.5f %.5f %.5f\" sort id append yes")
    ...
    if loop_extrusion_dynamics:
        ...
        for LES in range(1,maximum_number_of_extruded_particles):
