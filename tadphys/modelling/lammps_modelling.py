...
from random import uniform, randint, seed, random, sample, shuffle
...
def lammps_simulate(lammps_folder, run_time,
                    ...
                    loop_extrusion_dynamics=None, cleanup = False,
...
def run_lammps(kseed, lammps_folder, run_time,
    ...
    if compartmentalization:
        ...
        for pair in compartmentalization['interactions']:
            ...
            print("pair_coeff %s %s lj/cut %s %s %s" % (t1,t2,epsilon,sigma,rc))
    ...
    if loop_extrusion_dynamics:
        ...
        for LES in range(int(run_time/loop_extrusion_dynamics['extrusion_time'])):
            ...
            for extruder in range(len(extruders_positions)):
                ...
                # If the right part reaches the end of the chromosome -> Stop extruding
                if extruders_positions[extruder][1] < chromosome_particle_numbers[0]:
                    random_number = uniform(0, 1)
                ...
                if loop_extrusion_dynamics['barriers_permeability'] < 1.0:                    
                    ...
                    if extruders_positions[extruder][0] in loop_extrusion_dynamics['barriers']:
                        ...
                        if random_number >= loop_extrusion_dynamics['barriers_permeability']:
                            extruders_positions[extruder][0] += 1
                    # If the barrier is on the left of the extruders, which is extruding with the chain index, we have to re-put the extruder backwards
    ...
    for stg in range(len(xc)):
        ...
        for timepoint in range(1,timepoints+1):
            lammps_model = LAMMPSmodel({'x'          : [],
                                        ...
                                        'objfun'     : log_objfun[-1],
                                        'radius'     : 0.5, #float(CONFIG.HiC['resolution'] * CONFIG.HiC['scale'])/2,
                                        'index'  : kseed+timepoint,
    ...
    for cnt in range(number_of_conformations):
        ...
        while particle_inside == 0 or particles_overlap == 0:
            ...
            if particles_overlap != 0:
                for r in xrange(len(final_rosettes)):
...
def draw_loop_extrusion_starting_point(chromosome_length):

    # draw a starting point for extrusion along the chromosome
    random_particle =  randint(1, chromosome_length-1)
